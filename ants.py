...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place_number = 0
        moving_places = self.place
        while moving_places is not beehive:
            if place_number >= self.min_range and place_number <= self.max_range:
                if moving_places.bees != []:
                    return random_or_none(moving_places.bees)
            moving_places = moving_places.entrance
            place_number += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        #use method already assigned in ants
...
class ScubaThrower(ThrowerAnt):
    '''Same as Thrower but no lose armor when placed in water'''
    name = 'Scuba'
    food_cost = 6
    is_watersafe = True
    implemented = True
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.true_queen == False:
            ...
            moving_places = self.place.exit
            while moving_places:
                if moving_places.ant:
                        if moving_places.ant.is_container:
                            if moving_places.ant not in self.is_doubled:
                                moving_places.ant.damage *= 2
                                self.is_doubled.append(moving_places.ant)
                            if moving_places.ant.contained_ant and moving_places.ant.contained_ant not in self.is_doubled:
                                moving_places.ant.contained_ant.damage *= 2
                                self.is_doubled.append(moving_places.ant.contained_ant)
                            
                        elif moving_places.ant not in self.is_doubled:
                            moving_places.ant.damage *= 2
                            self.is_doubled.append(moving_places.ant)
                moving_places = moving_places.exit
